    ...
    def _action_done(self):
        ...
        for picking in self:
            if picking._invoice_at_shipping():
                picking.with_user(
                    picking.sale_id.partner_invoice_id.invoicing_user_id
                    or picking.company_id.invoicing_default_user_id
                    or self.env.user
                ).with_delay(
                    eta=datetime.datetime.now() + datetime.timedelta(0, 0, 0, 0, 0, 4)
                )._invoicing_at_shipping()
        return res
